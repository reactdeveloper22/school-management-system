...
if(isset($_POST['update_product'])){
   ...
   $update_product = $conn->prepare("UPDATE `teacher` SET `fname`= ?,`lname`= ?,`age`= ?,`email`= ?,`dateOfbirth`= ?,`home`= ?,`mobile`= ?, `idNum`= ?,`address`= ?,`postal`= ?,`nationality`= ? WHERE id = ?");
   $update_product->execute([$fname, $lname, $age, $email, $dateOf, $home, $mobile, $idNum, $address, $postal, $nationality, $pid]);

   $message[2] = 'Teacher updated successfully!';
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `teacher` SET image = ? WHERE id = ?");
...
<h1 class="tittleclass">Update Teacher's</h1>
<section class="update-product">

   <?php
      $update_id = $_GET['update'];
      $select_products = $conn->prepare("SELECT * FROM `teacher` WHERE id = ?");
...
<div class="containerr">
    ...
    <tr>
        <td><input type="submit" class="btn" value="Update Teacher" name="update_product"></td>
    </tr>
</div>
    </form>
</table>
